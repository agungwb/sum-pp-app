...
import { RepaymentReceiptRequest, RepaymentReceiptEditResponse } from '../dtos/repayment-receipt.dto';
...
export const repaymentReceiptService = {
  ...
  createReceipt: async (scheduleId: string, data: RepaymentReceiptRequest) => {
  ...
  updateReceipt: async (receiptId: string, data: Partial<RepaymentReceiptRequest>) => {
  ...
  getReceiptEditResponse: async (receiptId: string): Promise<ApiResponse<RepaymentReceiptEditResponse>> => {
